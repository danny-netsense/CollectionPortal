...
namespace CollectionPortal.Module.BusinessObjects
{
    ...
    { // Inherit from a different class to provide a custom primary key, concurrency and deletion behavior, etc. (https://documentation.devexpress.com/eXpressAppFramework/CustomDocument113146.aspx).
        ...
        public override void AfterConstruction()
        {
            ...
            _SalesOrderID = (Session.Evaluate<SalesOrder>(CriteriaOperator.Parse("Max(SalesOrderID)"), CriteriaOperator.Parse("")) == DBNull.Value ? 0 : Convert.ToInt32(Session.Evaluate<SalesOrder>(CriteriaOperator.Parse("Max(SalesOrderID)"), CriteriaOperator.Parse("")))) + 1;
        ...
        [PersistentAlias("_SalesOrderID")] // This line for read-only column mapping
        [Browsable(false)]
        ...
        [RuleRequiredField(DefaultContexts.Save)] // Validation for Required
        [Appearance("DocumentNoSalesOrderDetailsCond", Enabled = false, Criteria = "IsNullOrEmpty(DocumentNumber)", Context = "DetailView")]
        [Appearance("CustomerSalesOrderDetailsCond", Enabled = false, Criteria = "IsNullOrEmpty(Customer)", Context = "DetailView")]
        [Appearance("DocumentStatusSalesOrderDetailsCond", Enabled = false, Criteria = "DocumentStatus=3", Context = "DetailView")]
        ...
        public XPCollection<SalesOrderDetail> SalesOrderDetails
        {
            get
            {
                ...
                return GetCollection<SalesOrderDetail>(nameof(SalesOrderDetails));
            }
        }
        ...
        private void GetDocumentNumbering()
        {
            if (Location != null)
            {
                ...
                docnos.Criteria = CriteriaOperator.Parse("[Location]=? and [Module]=?", Location, Modules.SalesOrder);
                if (docnos.Count == 1)
                {
        ...
        protected override void OnSaving()
        {
            if (!(this.Session is NestedUnitOfWork) && Session.IsNewObject(this))
            {
                ...
                if (this.IsNew == true)
                {
                    ...
                    int slno = 0;
                    Guid unique_number;
                    foreach (SalesOrderDetail sod in this.SalesOrderDetails)
                    {
                        slno += 1;
                        unique_number = Guid.NewGuid();

                        insertDetailSQL += " insert into SORDDET";
                        insertDetailSQL += " (agentcode,sy_obj_name, sy_docno,  DOC_NO, DOC_DATE,  ";
                        insertDetailSQL += " DESCRIPT,  RATE, MRP, TOT_AMT,";
