...
namespace CollectionPortal.Module.BusinessObjects
{
    ...
    public class CustomerReceipt : BaseObject
    { // Inherit from a different class to provide a custom primary key, concurrency and deletion behavior, etc. (https://documentation.devexpress.com/eXpressAppFramework/CustomDocument113146.aspx).
        public CustomerReceipt(Session session)
            : base(session)
        {
        }
        public override void AfterConstruction()
        {
            ...
            _CustomerReceiptID = (Session.Evaluate<CustomerReceipt>(CriteriaOperator.Parse("Max(CustomerReceiptID)"), CriteriaOperator.Parse("")) == DBNull.Value ? 0 : Convert.ToInt32(Session.Evaluate<CustomerReceipt>(CriteriaOperator.Parse("Max(CustomerReceiptID)"), CriteriaOperator.Parse("")))) + 1;
        ...
        [Persistent("CustomerReceiptID")] // this line for read-only columns mapping
        private int _CustomerReceiptID;
        [RuleRequiredField] // Validation for Required
        [RuleUniqueValue] // Validation for unique va lue
        [PersistentAlias("_CustomerReceiptID")] // This line for read-only column mapping
        [Browsable(false)]
        public int CustomerReceiptID
        {
            get { return _CustomerReceiptID; }
        }
        ...
        [ImmediatePostData(true)]
        [Appearance("DocumentNoCashBankCond", Enabled = false, Criteria = "IsNullOrEmpty(DocumentNumber)", Context = "DetailView")]
        ...
        private XPCollection<Instrument> fAvailableInstruments;
        [Browsable(false)] // Prohibits showing the AvailableAccessories collection separately
        public XPCollection<Instrument> AvailableInstruments
        {
            get
            {
                ...
                {
                    // Retrieve all Sample objects
                    fAvailableInstruments = new XPCollection<Instrument>(Session);
                    // Filter the retrieved collection according to current conditions
                    RefreshInstrument();
                }
                // Return the filtered collection of Sample objects
                return fAvailableInstruments;
            }
        }
        ...
        [Appearance("DocumentNoInstrumentCond", Enabled = false, Criteria = "IsNullOrEmpty(DocumentNumber)", Context = "DetailView")]
        [DataSourceProperty(nameof(AvailableInstruments))]
        ...
        [Appearance("DocumentNoInstrumentNumberCond", Enabled = false, Criteria = "IsNullOrEmpty(DocumentNumber)", Context = "DetailView")]
        [Appearance("IsCashInstrumentNumberCond", Enabled = false, Criteria = "IsCash = 'true'", Context = "DetailView")]
        ...
        [Appearance("DocumentNoInstrumentDateCond", Enabled = false, Criteria = "IsNullOrEmpty(DocumentNumber)", Context = "DetailView")]
        ...
        string fChequeOn;
        [Appearance("DocumentNoChequeOnCond", Enabled = false, Criteria = "IsNullOrEmpty(DocumentNumber)", Context = "DetailView")]
        public string ChequeOn
        {
            get { return fChequeOn; }
            set { SetPropertyValue<string>("ChequeOn", ref fChequeOn, value); }
        }
        ...
        public XPCollection<CustomerReceiptDetail> CustomerReceiptDetails
        {
            get
            {
                return GetCollection<CustomerReceiptDetail>(nameof(CustomerReceiptDetails));
            }
        }
        ...
        private void GetDocumentNumbering()
        {
            if (Location != null)
            {
                ...
                docnos.Criteria = CriteriaOperator.Parse("[Location]=? and [Module]=?", Location, Modules.CustomerReceipt);
                if (docnos.Count == 1)
                {
        ...
        public void UpdateTotalAmount(bool forceChangeEvents)
        {
            ...
            foreach (CustomerReceiptDetail detail in CustomerReceiptDetails)
                tempTotalAmount += detail.AdjustAmount;
